...
void twi_init(void) {
    twi_state = TWI_READY;
    twi_inRepStart = 0;
    PORTC |= 1<<PORTC4; // Activate SDA internal resistance
    PORTC |= 1<<PORTC5; // Activate SCL internal resistance
    TWSR &= ~(1<<TWPS0 | 1<<TWPS1); // Clear prescalar
    TWBR = ((F_CPU / TWI_FREQ) - 16) / 2; // Set bit rate
    TWCR = (1<<TWEA)|(1<<TWEN)|(1<<TWIE); // Enable TWI, Interrupt, & Acknowledgements.
}
